...
function listarArticulos()
{
	...
	{
		destroy:true,
    dom: 'lfrtip',//Definimos los elementos del control de tabla
    language: {
      processing: "<i class='fa fa-spinner fa-2x fa-spin fa-fw' aria-hidden='true'></i>",
      search: "<i class='fa fa-search' aria-hidden='true'></i>",
      lengthMenu:     "Mostrando _MENU_ categorias",
...
function agregarDetalle(idarticulo,articulo)
{
       ...
       if ($('table#detalles tbody tr').length > 0)
        {
            ...
            for (var i = 0; i < idarticulo.length; i++) {
            	alert(idarticulo[i].value);
            	if(idarticulo[i].value != idarticulo)
            	{
    	        ...
    	        var fila='<tr class="filas" id="fila'+cont+'">'+
    	         ...
    	         '<td><button type="button" onclick="modificarSubototales()" class="btn btn-info btn-xs"><i class="fa fa-refresh"></i></button></td>'+
    	         '</tr>';
    	         cont++;
    	        detalles=detalles+1;
            ...
            {
                if ($(this).find('idarticulo[]').val() == idarticulo)
                {
        ...
        else
        {
           ...
           /* var nFilas = $("#detalles tr").length;
            var tableReg = document.getElementById('#detalles tr');
            //alert(nFilas);
            var i=0;
            if (nFilas > 2) 
            {
            	...
            	$("tbody tr").each(function()
            	{
                 ...
                 itemesorder.idarticulo = tds.filter(":eq(0)").text();
                // itemesorder.Articulo = tds.filter(":eq(1)").text();
                 items.push(itemesorder);
            	});
            	alert('el articulo ya ha sido agrado al detalles')
               
            } //de lo contario agregar fila
            else
            {
  ...
  function f_valida_repetido(idarticulo){
        if ($('table#detalles tbody tr').length > 0)
        {
            var v_valor = 0;
            $('table#detalles tbody tr').each(function()
            {
                if ($(this).find('idarticulo[]').val() == $("idarticulo").val())
                {
                    alert('valor repetido');
                    v_valor = 1;
                }
